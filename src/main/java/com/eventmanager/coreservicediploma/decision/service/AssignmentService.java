...
public class AssignmentService {
    ...
    public Map<Specification, UserInfo> getSolution(List<Specification> specs, Date date) {
        ...
        List<UserInfo> userInfos = calendars.stream().map(Calendar::getUserInfo).toList();

        //Вертикаль - worker
        int specificationCount = specs.size();
        //Горизонталь - task
        int userCount = userInfos.size();


        double[][] costs = new double[specificationCount][userCount];
        ...
        for (int i = 0; i < specificationCount; i++)
        {
            ...
            for (int j = 0; j < userCount; j++)
            {
                ...
                costs[i][j] = currentCalendar.getPrice();
            }
        }
